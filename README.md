...
## 概要
長期間アルバイトで務めていた小売り店に契約社員として入社したが、SESのインフラエンジニアへ転職し、主にSI系の様々なプロジェクトに参画していました。  
利用する技術スタックは様々で、Windows ServerをはじめとしたMicrosoft関連のソリューションであったり、Linuxサーバーとオープンソースを利用した環境もありました。
